...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeTNXAmount"))
        settings.setValue("nAnonymizeTNXAmount", 1000);

    nAnonymizeTNXAmount = settings.value("nAnonymizeTNXAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeTNXAmount"))
        SoftSetArg("-anonymizeTNXamount", settings.value("nAnonymizeTNXAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeTNXAmount:
            return QVariant(nAnonymizeTNXAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeTNXAmount:
            nAnonymizeTNXAmount = value.toInt();
            settings.setValue("nAnonymizeTNXAmount", nAnonymizeTNXAmount);
            emit anonymizeTNXAmountChanged(nAnonymizeTNXAmount);
            break;
        case CoinControlFeatures:
