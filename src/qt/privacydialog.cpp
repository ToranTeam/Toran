                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zTNX ought to be enough for anybody." - Bill Gates, 2017
    ui->zTNXpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzTNXSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zTNXpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzTNX_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zTNX is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        ...
        if (!ctx.isValid()) {
            ...
            ui->TEMintStatus->setPlainText(tr("Error: Your wallet is locked. Please enter the wallet passTNXase first."));
            return;
        }
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zTNX...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zTNX in ") +
...
void PrivacyDialog::on_pushButtonSpendzTNX_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zTNX is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zTNX
        sendzTNX();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zTNX
    sendzTNX();
}

void PrivacyDialog::on_pushButtonZTNXControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ZTNXControlDialog* zTNXControl = new ZTNXControlDialog(this);
    zTNXControl->setModel(walletModel);
    zTNXControl->exec();
}

void PrivacyDialog::setZTNXControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzTNXSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzTNX()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!IsValidDestinationString(ui->payTo->text().toStdString())) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid TNX Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zTNXpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zTNX is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " TNX </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zTNX</b>";
    ...
    // use mints from zTNX selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZTNXControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZTNXControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zTNX require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zTNX"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == ZTNX_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zTNX require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zTNX"));
            ui->TEMintStatus->repaint();
            return;
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zTNX was spent successfully update the addressbook with the label
    ...
    // Clear zTNX selector in case it was used
    ZTNXControlDialog::setSelectedMints.clear();
    ui->labelzTNXSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zTNX Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " TNX, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zTNX Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zTNXTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zTNX </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zTNX "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zTNX "));
    ui->labelzTNXAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzTNXSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzTNX->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zTNX
        ui->pushButtonMintzTNX->setEnabled(false);
        ui->pushButtonMintzTNX->setToolTip(tr("zTNX is currently disabled due to maintenance."));

        // Spend zTNX
        ui->pushButtonSpendzTNX->setEnabled(false);
        ui->pushButtonSpendzTNX->setToolTip(tr("zTNX is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zTNX
        ui->pushButtonMintzTNX->setEnabled(true);
        ui->pushButtonMintzTNX->setToolTip(tr("PrivacyDialog", "Enter an amount of TNX to convert to zTNX", 0));

        // Spend zTNX
        ui->pushButtonSpendzTNX->setEnabled(true);
        ui->pushButtonSpendzTNX->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}