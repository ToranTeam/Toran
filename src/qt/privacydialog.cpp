                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zSLTC ought to be enough for anybody." - Bill Gates, 2017
    ui->zSLTCpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzSLTCSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zSLTCpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzSLTC_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zSLTC is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        ...
        if (!ctx.isValid()) {
            ...
            ui->TEMintStatus->setPlainText(tr("Error: Your wallet is locked. Please enter the wallet passSLTCase first."));
            return;
        }
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zSLTC...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zSLTC in ") +
...
void PrivacyDialog::on_pushButtonSpendzSLTC_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zSLTC is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zSLTC
        sendzSLTC();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zSLTC
    sendzSLTC();
}

void PrivacyDialog::on_pushButtonZSLTCControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ZSLTCControlDialog* zSLTCControl = new ZSLTCControlDialog(this);
    zSLTCControl->setModel(walletModel);
    zSLTCControl->exec();
}

void PrivacyDialog::setZSLTCControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzSLTCSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzSLTC()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!IsValidDestinationString(ui->payTo->text().toStdString())) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid SLTC Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zSLTCpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zSLTC is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " SLTC </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zSLTC</b>";
    ...
    // use mints from zSLTC selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZSLTCControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZSLTCControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zSLTC require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zSLTC"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == ZSLTC_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zSLTC require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zSLTC"));
            ui->TEMintStatus->repaint();
            return;
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zSLTC was spent successfully update the addressbook with the label
    ...
    // Clear zSLTC selector in case it was used
    ZSLTCControlDialog::setSelectedMints.clear();
    ui->labelzSLTCSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zSLTC Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " SLTC, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zSLTC Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zSLTCTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zSLTC </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zSLTC "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zSLTC "));
    ui->labelzSLTCAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzSLTCSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzSLTC->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zSLTC
        ui->pushButtonMintzSLTC->setEnabled(false);
        ui->pushButtonMintzSLTC->setToolTip(tr("zSLTC is currently disabled due to maintenance."));

        // Spend zSLTC
        ui->pushButtonSpendzSLTC->setEnabled(false);
        ui->pushButtonSpendzSLTC->setToolTip(tr("zSLTC is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zSLTC
        ui->pushButtonMintzSLTC->setEnabled(true);
        ui->pushButtonMintzSLTC->setToolTip(tr("PrivacyDialog", "Enter an amount of SLTC to convert to zSLTC", 0));

        // Spend zSLTC
        ui->pushButtonSpendzSLTC->setEnabled(true);
        ui->pushButtonSpendzSLTC->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}