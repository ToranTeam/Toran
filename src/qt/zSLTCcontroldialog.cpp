...
#include "ui_zSLTCcontroldialog.h"
...
std::set<std::string> ZSLTCControlDialog::setSelectedMints;
std::set<CMintMeta> ZSLTCControlDialog::setMints;

ZSLTCControlDialog::ZSLTCControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZSLTCControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZSLTCControlDialog::~ZSLTCControlDialog()
{
    delete ui;
}

void ZSLTCControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZSLTCControlDialog::updateList()
{
...
void ZSLTCControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZSLTCControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZSLTC_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZSLTCControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZSLTCControlDialog::GetSelectedMints()
{
...
void ZSLTCControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
