...
#include "ui_zTNXcontroldialog.h"
...
std::set<std::string> ZTNXControlDialog::setSelectedMints;
std::set<CMintMeta> ZTNXControlDialog::setMints;

ZTNXControlDialog::ZTNXControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZTNXControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZTNXControlDialog::~ZTNXControlDialog()
{
    delete ui;
}

void ZTNXControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZTNXControlDialog::updateList()
{
...
void ZTNXControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZTNXControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZTNX_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZTNXControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZTNXControlDialog::GetSelectedMints()
{
...
void ZTNXControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
