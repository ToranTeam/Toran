...
void ComputePreFactor(std::string strPassSLTCase, std::string strSalt, uint256& prefactor)
{
    //passfactor is the scrypt hash of passSLTCase and ownersalt (NOTE this needs to handle alt cases too in the future)
    uint64_t s = uint256(ReverseEndianString(strSalt)).Get64();
    scrypt_hash(strPassSLTCase.c_str(), strPassSLTCase.size(), BEGIN(s), strSalt.size() / 2, BEGIN(prefactor), 16384, 8, 8, 32);
}
...
std::string BIP38_Encrypt(std::string strAddress, std::string strPassSLTCase, uint256 privKey, bool fCompressed)
{
    string strAddressHash = AddressToBip38Hash(strAddress);

    uint512 hashed;
    uint64_t salt = uint256(ReverseEndianString(strAddressHash)).Get64();
    scrypt_hash(strPassSLTCase.c_str(), strPassSLTCase.size(), BEGIN(salt), strAddressHash.size() / 2, BEGIN(hashed), 16384, 8, 8, 64);
...
bool BIP38_Decrypt(std::string strPassSLTCase, std::string strKey, uint256& privKey, bool& fCompressed)
{
    ...
    uint256 prefactor;
    ComputePreFactor(strPassSLTCase, prefactorSalt, prefactor);
