...
#ifndef SLTC_ZSLTCWALLET_H
#define SLTC_ZSLTCWALLET_H
...
public:
    CzSLTCWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZSLTC(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZSLTC(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //SLTC_ZSLTCWALLET_H
