...
CzSLTCTracker::CzSLTCTracker(std::string strWalletFile)
{
...
CzSLTCTracker::~CzSLTCTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzSLTCTracker::Init()
{
...
bool CzSLTCTracker::Archive(CMintMeta& meta)
{
...
bool CzSLTCTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzSLTCTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzSLTCTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzSLTCTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzSLTCTracker::GetSerialHashes()
{
...
CAmount CzSLTCTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzSLTCTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzSLTCTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzSLTCTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzSLTCTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzSLTCTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzSLTCTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzSLTCTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzSLTCTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzSLTCTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzSLTCTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzSLTCTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzSLTCTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzSLTCTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzSLTCTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzSLTCTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzSLTCTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzSLTC from DB\n", __func__, listDeterministicDB.size());
    }
