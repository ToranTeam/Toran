...
#ifndef TNX_ZTNXWALLET_H
#define TNX_ZTNXWALLET_H
...
public:
    CzTNXWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZTNX(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZTNX(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //TNX_ZTNXWALLET_H
