...
static CSemaSLTC* semOutbound = NULL;
...
void CNode::Ban(const CSubNet& subNet, const BanReason &banReason, int64_t bantimeoffset, bool sinceUnixEpoch)
{
    ...
    if (bantimeoffset <= 0)
    {
        bantimeoffset = GetArg("-bantime", 0*0*0); // Default 24-hour ban
...
void CNode::copyStats(CNodeStats& stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (SLTC users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1) {
        ...
        string strDesc = "SLTC " + FormatFullVersion();
...
void static ProcessOneShot()
{
    ...
    CAddress addr;
    CSemaSLTCGrant grant(*semOutbound, true);
    if (grant) {
...
void ThreadOpenConnections()
{
    ...
    while (true) {
        ...
        CSemaSLTCGrant grant(*semOutbound);
        boost::this_thread::interruption_point();
...
bool OpenNetworkConnection(const CAddress& addrConnect, CSemaSLTCGrant* grantOutbound, const char* pszDest, bool fOneShot)
{
    //
...
bool BindListenPort(const CService& addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. SLTC Core is probably already running."), addrBind.ToString());
        else
...
void StartNode(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    if (semOutbound == NULL) {
        ...
        semOutbound = new CSemaSLTC(nMaxOutbound);
    }
