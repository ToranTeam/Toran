...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the TNX and zTNX money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 16208, 11773));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, TNX, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all TNX specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:16208"));
    ...
    strUsage += HelpMessageOpt("-backupzTNX=<n>", strprintf(_("Enable automatic wallet backups triggered after each zTNX minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zTNXbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zTNX backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif
//    strUsage += "  -anonymizeTNXamount=<n>     " + strprintf(_("Keep N TNX anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 16209, 11774));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The TNX Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. TNX Core is shutting down."));
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. TNX Core is probably already running."), strDataDir));
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //TNX specific: zerocoin and spork DB's
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZTNXMinted();
                        RecalculateZTNXSpent();
                    }
                    RecalculateTNXSupply(1);
                }
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of TNX Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart TNX Core to complete") << "\n";
        ...
        zwalletMain = new CzTNXWallet(pwalletMain->strWalletFile);
        ...
        uiInterface.InitMessage(_("Syncing zTNX wallet..."));

        bool fEnableZTNXBackups = GetBoolArg("-backupzTNX", true);
        pwalletMain->setZTNXAutoBackups(fEnableZTNXBackups);
...
//    }
//
//    nAnonymizeTNXAmount = GetArg("-anonymizeTNXamount", 0);
//    if (nAnonymizeTNXAmount > 999999) nAnonymizeTNXAmount = 999999;
//    if (nAnonymizeTNXAmount < 2) nAnonymizeTNXAmount = 2;
    ...
    LogPrintf("Anonymize TNX Amount %d\n", nAnonymizeTNXAmount);
    ...
    /* Denominations
       ...
       For example:
       1TNX+1000 == (.1TNX+100)*10
       10TNX+10000 == (1TNX+1000)*10
    */
