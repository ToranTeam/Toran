...
CzTNXTracker::CzTNXTracker(std::string strWalletFile)
{
...
CzTNXTracker::~CzTNXTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzTNXTracker::Init()
{
...
bool CzTNXTracker::Archive(CMintMeta& meta)
{
...
CMintMeta CzTNXTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzTNXTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzTNXTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzTNXTracker::GetSerialHashes()
{
...
CAmount CzTNXTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzTNXTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzTNXTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzTNXTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzTNXTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzTNXTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzTNXTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzTNXTracker::HasSerialHash(const uint256& hashSerial) const
{
...
void CzTNXTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzTNXTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzTNXTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzTNXTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
std::set<CMintMeta> CzTNXTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzTNX from DB\n", __func__, listDeterministicDB.size());
    }
