...
CzSLTCWallet::CzSLTCWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zSLTC seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZSLTCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZSLTCSeed_deprecated()) {
                    LogPrintf("%s: Updated zSLTC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zSLTC seed\n", __func__);
                }
            }
        }
    }
...
bool CzSLTCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZSLTCCount(nCountLastUsed);
    else if (!walletdb.ReadZSLTCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzSLTCWallet::Lock()
{
    seedMaster = 0;
}

void CzSLTCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzSLTCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZSLTC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzSLTCWallet::LoadMintPoolFromDB()
{
...
void CzSLTCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzSLTCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzSLTCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zSLTCTracker->HasPubcoinHash(pMint.first)) {
...
bool CzSLTCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZSLTC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zSLTCTracker which also adds to database
    pwalletMain->zSLTCTracker->Add(dMint, true);
...
void CzSLTCWallet::SeedToZSLTC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzSLTCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzSLTCWallet::GenerateDeterministicZSLTC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzSLTCWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzSLTCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
