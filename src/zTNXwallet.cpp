...
CzTNXWallet::CzTNXWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zTNX seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZTNXSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZTNXSeed_deprecated()) {
                    LogPrintf("%s: Updated zTNX seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zTNX seed\n", __func__);
                }
            }
        }
    }
...
bool CzTNXWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZTNXCount(nCountLastUsed);
    else if (!walletdb.ReadZTNXCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzTNXWallet::Lock()
{
    seedMaster = 0;
}

void CzTNXWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzTNXWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZTNX(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzTNXWallet::LoadMintPoolFromDB()
{
...
void CzTNXWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzTNXWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzTNXWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zTNXTracker->HasPubcoinHash(pMint.first)) {
...
bool CzTNXWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZTNX(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zTNXTracker which also adds to database
    pwalletMain->zTNXTracker->Add(dMint, true);
...
void CzTNXWallet::SeedToZTNX(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzTNXWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzTNXWallet::GenerateDeterministicZTNX(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzTNXWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzTNXWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
